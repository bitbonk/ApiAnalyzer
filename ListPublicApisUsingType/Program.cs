...
if (args.Length == 0)
{
    Console.WriteLine("Usage: ListPublicApisUsingType <solution-path>");
    return;
}
...
var publicMembers =
    new Dictionary<INamedTypeSymbol, List<ISymbol>>(SymbolEqualityComparer.Default);
            ...
            if (typeSymbol is {DeclaredAccessibility: Accessibility.Public, ContainingType: null})
                foreach (var publicMember in typeSymbol.GetMembers()
                             .Where(member => member.DeclaredAccessibility == Accessibility.Public
                                              && member.ToDisplayString().Contains("DateTime",
...
var output = "c:/temp/public-members-with-date-time.md";
